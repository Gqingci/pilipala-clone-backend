...
public class ABaseController {
    ...
    protected void saveToken2Cookie(HttpServletResponse response, String token) {
        Cookie cookie = new Cookie(RedisConstants.TOKEN_WEB, token);
        cookie.setMaxAge(RedisConstants.TIME_SECONDS_DAY);
        cookie.setPath("/");
        response.addCookie(cookie);
    }

    protected UserTokenInfoDTO getUserTokenInfoDTO() {
    ...
    public UserTokenInfoDTO getTokenInfoFromCookie() {
        HttpServletRequest request = ((ServletRequestAttributes) RequestContextHolder.getRequestAttributes()).getRequest();
