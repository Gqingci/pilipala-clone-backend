...
public class RedisComponent {
    ...
    // 更新用户的Token
    public void updateToken(UserTokenInfoDTO userTokenInfoDTO) {
        redisUtils.setex(RedisConstants.REDIS_WEB_TOKEN_KEY + userTokenInfoDTO.getToken(), userTokenInfoDTO, RedisConstants.LOGIN_USER_TTL);
    }
    ...
    public void saveSysSettingDTO(SysSettingDTO sysSettingDTO) {
        redisUtils.set(RedisConstants.REDIS_SYS_SETTING_KEY, sysSettingDTO);
    }
    ...
    public void addKeywordCount(String keyword) {
        redisUtils.zaddCount(RedisConstants.REDIS_VIDEO_SEARCH_COUNT_KEY, keyword);
    }

    public List<String> getKeywordTop(Integer top) {
        return redisUtils.getZSetList(RedisConstants.REDIS_VIDEO_SEARCH_COUNT_KEY, top - 1);
    }

    public void addVideoPlay(VideoPlayInfoDTO videoPlayInfoDTO) {
        redisUtils.lpush(RedisConstants.REDIS_VIDEO_PLAY_QUEUE_KEY, videoPlayInfoDTO, null);
    }

    public VideoPlayInfoDTO getVideoPlayFromVideoPlayQueue() {
        return (VideoPlayInfoDTO) redisUtils.rpop(RedisConstants.REDIS_VIDEO_PLAY_QUEUE_KEY);
    }
    ...
    public void recordVideoPlay(String videoId) {
        ...
        redisUtils.incrementex(RedisConstants.REDIS_VIDEO_PLAY_COUNT_KEY + date + ":" + videoId, RedisConstants.REDIS_KEY_EXPIRES_ONE_DAY * 2);
    }

    public Map<String, Integer> getVideoPlayCount(String date) {
        return redisUtils.getBatch(RedisConstants.REDIS_VIDEO_PLAY_COUNT_KEY + date);
    }
    ...
    public void set(String key, String value, long secondsUntilEndOfDay) {
        redisUtils.setex(key, value, secondsUntilEndOfDay);
    }
}
