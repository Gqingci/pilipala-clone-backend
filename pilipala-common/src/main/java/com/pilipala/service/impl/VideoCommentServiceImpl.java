...
public class VideoCommentServiceImpl implements VideoCommentService {
    ...
    public void deleteComment(Integer commentId, String id) {
        ...
        if (id != null && !video.getUserId().equals(id) && !videoComment.getUserId().equals(id)) {
            throw new BusinessException(ResponseCodeEnum.CODE_600);
        }
        ...
        if (videoComment.getPCommentId() == 0) {
            // 一级评论，递归删除所有子评论
            deleteCount = deleteCommentRecursively(commentId);
        } else {
            // 二级或更深层，只删除自己
    ...
    private int deleteCommentRecursively(Integer commentId) {
        int count = 0;
        VideoCommentQuery query = new VideoCommentQuery();
        query.setPCommentId(commentId);

        List<VideoComment> childComments = videoCommentMapper.selectListWithChildren(query);
        for (VideoComment child : childComments) {
            count += deleteCommentRecursively(child.getCommentId());
        }
